...
import { db } from "@nullstack/database/client";
import type { Prisma } from "@nullstack/database";
import {
  getToolContracts,
  modeSchema,
  type ModeType,
...
type nullstackUIMessage = UIMessage<
  ChatMessageMetadata,
  never,
...
const submitSchema = z.object({
  ...
  messages: z
    .array(
      z.custom<nullstackUIMessage>((value) => {
        return (
          value != null &&
          typeof value === "object" &&
          "id" in value &&
...
function hasPendingToolCalls(message: nullstackUIMessage) {
...
const app = new Hono<AuthenticatedEnv>().post(
  ...
  async (c) => {
    ...
    const previousMessages = Array.isArray(session.messages)
      ? (session.messages as unknown as nullstackUIMessage[])
      : [];
    ...
    const nextMessages = await validateUIMessages<nullstackUIMessage>({
    ...
    return result.toUIMessageStreamResponse<nullstackUIMessage>({
